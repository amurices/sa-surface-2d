...
// negative for clockwise turn, and zero if the point_ts are collinear.
    ...
    point_t findDirectionVector(const point_t &a, const point_t &b, const point_t &c, direction_t Type) {
        ...
        if (Type == ORTHOGONAL) {
            ...
            // Vd = P(n+1) - P(n-1)
            point_t dO(b.y - a.y, a.x - b.x);
            dO = dO * findNorm(dO);

            point_t aShift = a - c;
            point_t bShift = b - c;

            aShift = aShift * (1/findNorm(aShift));
            bShift = bShift * (1/findNorm(bShift));

            directionOffset = (aShift + bShift) * 0.5;
            directionOffset = directionOffset * (1/findNorm(directionOffset));
