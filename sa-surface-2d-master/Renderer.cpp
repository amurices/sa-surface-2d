...
void Renderer::render_text(FTGLPixmapFont &font, const char* text, point_t pos, int faceSize)
{
	...
	double mulX = pos.x + 1;
	double mulY = pos.y + 1;
    font.Render(text, -1, FTPoint((double) wWidth * mulX * 0.5, (double) wHeight * mulY * 0.5, 0.0));
	glMatrixMode(GL_PROJECTION);
	glPopMatrix();
	glMatrixMode(GL_MODELVIEW);
	glPopMatrix();
...
void Renderer::render_text(FTGLTextureFont &font, const char* text, point_t pos, int faceSize)
{
	glEnable(GL_TEXTURE_2D);
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	glMatrixMode(GL_PROJECTION);
