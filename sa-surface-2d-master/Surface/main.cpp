...
int main() {
  ...
  //  input.open("input.txt");
   ...
   // InitSaParams params;
	...
	FTGLPixmapFont fonti("/Windows/OpenSans-Regular.ttf");      // Load .ttf file to memory
	FTGLTextureFont fonty("/Windows/Fonts/arial.ttf");      // Load .ttf file to memory
	fonti.FaceSize(10);
	fonty.FaceSize(72);
	fonty.Render("Hello World");
    
    point_t mousePos;                                      // Mouse position for (minimal) input handling
    
	Renderer myRenderer(10); // Renderer instance (so it can store some loop-related stuff transparently)
	...
	//exit(EXIT_FAILURE);   // Font loading handling
    // -----------------------------------------
	...
	//	std::cout << gen << " currThick: " << currentThick << std::endl;

        std::sprintf(nrg, "nrg %.7f", optimizers[currentThick]->energy);
	//	std::cout << nrg << " currThick: " << currentThick << std::endl;

        std::sprintf(wht, "wht %.7f", optimizers[currentThick]->white);
	//	std::cout << wht << " currThick: " << currentThick << std::endl;
    ...
    //    myRenderer.render_intersections(is);
        ...
        for (ListDigraph::NodeIt no(optimizers[currentThick]->org->outer.graph); no != INVALID; ++no)
        {
            ...
            if (dist(mousePos, (*optimizers[currentThick]->org->outer.coords)[ no ]) < 0.05 && rendering < 1)
            {
                ...
                myRenderer.render_node(
					optimizers[currentThick]->org->inner.graph.nodeFromId(
						optimizers[currentThick]->org->outer.correspondence[optimizers[currentThick]->org->outer.graph.id(no)]
						), 
					optimizers[currentThick]->org->inner,
					triple_t (0.7, 0.7, 0.3)
				);
